<x-layouts.admin>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            Edit Section: {{ $class->name }} - {{ $section->name }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form action="{{ route('admin.sections.update', [$class, $section]) }}" method="POST">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="flex">
                                ...
                                <div class="ml-3">
                                    <p class="text-sm text-yellow-700">
                                        This section has {{ $section->students->count() }} student(s). Changes will affect existing students.
                                    </p>
                                </div>
                            </div>
                        </div>
