        ...
        <div class="card-premium p-6">
            <form method="GET" action="{{ route('admin.reports.fee-collection') }}" class="space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <!-- Payment Mode Filter -->
                    <div>
                        <label for="payment_mode" class="block text-sm font-medium text-slate-700 mb-1">Payment Mode</label>
                        <select name="payment_mode" id="payment_mode" class="block w-full rounded-xl border-slate-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm">
                    ...
                    <div class="space-y-3">
                        @foreach($collectionByFeeHead as $feeHead)
                            <div class="flex items-center justify-between">
                                ...
                                <span class="text-sm font-semibold text-slate-900">₹{{ number_format($feeHead->total, 2) }}</span>
                            </div>
                            <div class="w-full bg-slate-200 rounded-full h-2">
                                <div class="bg-primary-500 h-2 rounded-full" style="width: {{ $grandTotal > 0 ? ($feeHead->total / $grandTotal * 100) : 0 }}%"></div>
                            </div>
                        @endforeach
                    </div>
                </div>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-slate-200">
                    <thead class="bg-slate-50">
                        <tr>
                            <th class="px-6 py-3 text-left text-xs font-semibold text-slate-700 uppercase tracking-wider">Receipt No</th>
                            ...
                            <th class="px-6 py-3 text-center text-xs font-semibold text-slate-700 uppercase tracking-wider">Mode</th>
                        ...
                        @forelse($recentReceipts as $receipt)
                            <tr class="hover:bg-slate-50 transition-colors duration-150">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <a href="{{ route('admin.fee-receipts.show', $receipt) }}" class="text-primary-600 hover:text-primary-900">
                                        View
                                    </a>
                                </td>
                            </tr>
